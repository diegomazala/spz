...
With extension `SPZ_NIANTIC_georeference`, SPZ supports storing a similarity transform that places the asset on a planetary body: a translation `origin` (meters), a unit quaternion `rotation`, and a dimensionless uniform `scale` map local positions into a geocentric CRS identified by an `AUTHORITY:CODE` string (e.g. `EPSG:4978`, WGS84 ECEF) as `p_crs = scale * (q * p_local * q⁻¹) + origin`. `scale` corrects for the scale ambiguity inherent to unreferenced reconstruction (e.g. monocular SfM) and is uniform only — no anisotropic scale or shear is supported. An optional coordinate epoch (the decimal year the coordinates are valid at) and an optional `provenance_crs` recording the source compound CRS (provenance only — never a transform source), tagged WKT2 or PROJJSON can also be stored. The library never applies the transform to the Gaussian data itself. See [extensions/README.md](extensions/README.md) for the payload layout and validation rules.
